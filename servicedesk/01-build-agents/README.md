...
5. **Wire the three together locally** (before the full Foundry workflow in stage 4) — `agents.py` orchestrates Triage's decision into Remediation or Escalation directly, including falling through to Escalation if Remediation refuses a ticket:
   ```bash
   python agents.py --ticket INC0010236
   python agents.py --ticket INC0010238
   python agents.py --all          # run every sample ticket in one go
   ```
